...
interface CourseApprovedSuccessModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onRedirectAndClose: () => void;
}

export function CourseApprovedSuccessModal({
  open,
  onOpenChange,
  onRedirectAndClose,
}: CourseApprovedSuccessModalProps) {
  return (
      ...
      <DialogContent className="text-center w-full sm:w-[400px] h-auto sm:h-[290px] rounded-[12px] pt-6 sm:pt-[34px] px-4 sm:px-[24px] pb-6 sm:pb-[24px] mx-auto">
        <DialogHeader className="flex flex-col items-center gap-2">
          ...
          <DialogDescription className="mt-2 text-sm text-gray-600">
            You have successfully approved this course.
          </DialogDescription>
        </DialogHeader>
        <div className="mt-6">
          <Button
            onClick={onRedirectAndClose}
